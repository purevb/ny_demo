...
class NYAudioManager {
  ...
  void _performCleanup() {
    ...
    for (final entry in _cache.entries) {
      final timeSinceLastUse = now.difference(entry.value.lastUsed);
      if (timeSinceLastUse > cacheTimeout) {
    ...
    return {
      ...
      'usage': _cache.map(
        (key, value) => MapEntry(key, {
          ...
          'minutesSinceLastUse': DateTime.now()
              .difference(value.lastUsed)
              .inMinutes,
        }),
      ),
    };
  ...
  void clearCache() {
    try {
      ...
      _cache.clear();
      debugPrint('NYAudioManager: Cache cleared');
    } catch (e) {
      debugPrint('NYAudioManager: Error clearing cache - $e');
    }
  }
